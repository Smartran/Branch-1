import Head from 'next/head'
import Image from 'next/image'
import styles from '../styles/Home.module.css'
import Navbar from '../components/Navbar'
import Footer from '../components/Footer'
import Custom from '../components/Custom'
import Logo from '../public/Logo.png'


export default function marketplace() {
    return (
        <div>
            <Navbar />
            <div className='flex justify-center items-center'>
                <div className='text-3xl text-white font-bold mx-4 my-8 bg-gradient-to-r from-purple-700 to-pink-500  rounded-xl'>
                    <div className='mx-4 my-4'>
                        Purchase your own  money streaming NFTs !
                    </div>
                </div>
            </div>
            

            <div className='grid grid-cols-3 gap-2 '>
                <div className=' bg-black text-white mx-2 my-2 rounded-xl  ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 50 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer '>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>
                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 100 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>


                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 200 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>

                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 500 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>

                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 1000 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>



                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 1500 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>





                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 2000 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>



                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 2500 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>

                <div className=' bg-black text-white mx-2 my-2 rounded-xl ring-2 ring-pink-500 ring-inset'>
                <div className="mx-2 my-2">
                    <Image src={Logo} 
                    alt='' height="260px" width='400px'>
                    </Image>
                    </div>
                    <div className=' text-lg font-semibold mx-4'>
                        Name : 
                        </div>
                        <div className=' text-lg font-semibold mx-4'>
                        Worth : 3000 USDT 
</div>
<div className=' text-lg font-semibold mx-4'>
    Details : The NFT will stream the money which is equivalent to its worth
                    </div>
    <div className='flex justify-center items-center my-2'>
    <div className='rounded-3xl bg-gray-900  ring-1 ring-pink-500 ring-inset my-2'>
                    <div className='  text-purple-500 mx-10 my-4 font-bold text-2xl cursor-pointer'>
                        Buy
                    </div>
                    </div>
                    </div>
                </div>
            


            
                    </div>
        <Footer />
        </div>
        )
    }